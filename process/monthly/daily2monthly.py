...
for basin in basin_list:
  ...
  gauge_list = getList(root+'/'+basin+'/daily/row2col/*.out')

  for i,gauge in enumerate(gauge_list):

    # list of yearly files for this gauge
    gauge_fils = glob.glob(root+'/'+basin+'/daily/row2col/*'+gauge+'*.out')
    ...
    if not os.path.exists(root+'/'+basin+'/monthly/'):
      os.makedirs(root+'/'+basin+'/monthly/')
    outFil = root+'/'+basin+'/monthly/monthly_sl_'+basin+'_'+meta['number']+meta['version'].lower()+'_'+str(yr1)+'-'+str(yr2)+'.tsv'
  ...
  outFil = root+'/'+basin+'/monthly/'+basin+'_meta.tsv'
  print "Writing: %s" %outFil
